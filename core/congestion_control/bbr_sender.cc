...
void BbrSender::SetInitialCongestionWindowInPackets(
    ...
    //std::cout << "Selected BBR" << std::endl;
    QUIC_DVLOG(1) << "Selected BBR";
  if (mode_ == STARTUP) {
    ...
    std::cout<<"Congestion window SetInitialCongestionWindowInPackets"<<congestion_window_<<"\n";
  }
}
...
QuicByteCount BbrSender::GetCongestionWindow() const {
  QUIC_DVLOG(1) << "BbrSender::GetTargetCongestionWindow BBR";
  //std::cout << "BbrSender::GetCongestionWindow BBR" << std::endl;
  if (mode_ == PROBE_RTT) {
...
std::cout<<"Congestion window GetCongestionWindow()"<<congestion_window_<<"\n";
...
void BbrSender::SetFromConfig(const QuicConfig& config,
  ...
  if (config.HasClientRequestedIndependentOption(kMIN1, perspective)) {
    ...
    std::cout<<" Min CWND "<<min_congestion_window_<<"\n";
  }
...
void BbrSender::AdjustNetworkParameters(const NetworkParams& params) {
  ...
  if (mode_ == STARTUP) {
    ...
    if (new_cwnd < congestion_window_ && !params.allow_cwnd_to_decrease) {
      // Print the new congestion winow
      std::cout<<"In startup phage when new_cwnd < congestion_window_ && !params.allow_cwnd_to_decrease"<< congestion_window_ <<"\n";
    ...
    // Print the new congestion winow
    std::cout<<"when new_cwnd > congestion_window_ "<< congestion_window_ <<"\n";
...
void BbrSender::EnterStartupMode(QuicTime now) {
  ...
  //std::cout<<" cwnd in startup phage "<<congestion_window_gain_<<"\n";
}

void BbrSender::EnterProbeBandwidthMode(QuicTime now) {
  mode_ = PROBE_BW;
  congestion_window_gain_ = congestion_window_gain_constant_;
 // std::cout<<" cwnd in probe bandwidth phage "<<congestion_window_gain_<<"\n";
...
bool BbrSender::MaybeUpdateMinRtt(QuicTime now,
  ...
  if (min_rtt_expired || sample_min_rtt < min_rtt_ || min_rtt_.IsZero()) {
    QUIC_DVLOG(1) << "Min RTT updated, old value: " << min_rtt_ << "BBR "
...
void BbrSender::MaybeExitStartupOrDrain(QuicTime now) {
  if (mode_ == STARTUP && is_at_full_bandwidth_) {
    ...
    congestion_window_gain_ = high_cwnd_gain_;
      //std::cout<<" cwnd "<<congestion_window_gain_<<"\n";
  }
  if (mode_ == DRAIN &&
...
void BbrSender::CalculateCongestionWindow(QuicByteCount bytes_acked,
  ...
  //Modify print congestion window
  std::cout<<" PROBE_RTT STATE "<<congestion_window_<<"\n";
}
...
void BbrSender::CalculateRecoveryWindow(QuicByteCount bytes_acked,
  ...
  recovery_window_ = recovery_window_ >= bytes_lost
                         ...
                         : kMaxSegmentSize;
  std::cout<< "cwnd size: "<<recovery_window_<<"\n";
